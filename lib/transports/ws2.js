...
class WSv2 extends EventEmitter {
  ...
  _handleChannelMessage (msg, rawMsg) {
    ...
    if (msg[1] === 'hb') {
      if (channelData.channel === 'candles') {
        const internalMessage = [channelData.chanId, 'candle', ['hb']]
        internalMessage.filterOverride = [channelData.key]
        this._propagateMessageToListeners(internalMessage, channelData, false)
      }
  ...
  static _notifyListenerGroup (lGroup, msg, transform, ws, chanData) {
    ...
    const listeners = lGroup[eventName].filter((listener) => {
      const { filter, emitHeartBeat } = listener

      const heartBitFilter = data[0] === 'hb'
        ? emitHeartBeat === true
        : true
      ...
      return heartBitFilter && WSv2._payloadPassesFilter(filterByData, filter)
    })
   ...
   * @private
   */
  _registerListener (eventName, filter, modelClass, cbGID, cb, emitHeartBeat) {
    if (!cbGID) cbGID = null
  ...
  onCandle ({ key, cbGID, emitHeartBeat }, cb) {
    this._registerListener('candle', { 0: key }, Candle, cbGID, cb, emitHeartBeat)
  }
