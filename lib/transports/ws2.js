...
class WSv2 extends EventEmitter {
  ...
  /**
   * The closing and re-connection logic it buggy, making it impossible
   * to manually close while re-connection is in progress. This method
   * is a workaround.
   */
  async destroy (code, reason) {
  ...
  async _onWSClose () {
    ...
    if (!this._destoyed && (this._isReconnecting || (this._autoReconnect && !this._isClosing))) {
      ...
      this._reconnectTimeout = setTimeout(async () => {
        try {
          if (this._reconnectThrottler) {
            // TODO: this migth leave dangling timeouts. Need to find a way to clear them.
