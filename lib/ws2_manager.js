...
class WS2Manager extends EventEmitter {
  ...
  /**
   * Closes all open sockets (Also waits for pending subscriptions to resolve and closes them)
   *
   * @returns {Promise} p
   */
  async close () {
    const subCb = (pSub, resolve) => (msg = {}) => {
      const msgIdentifier = WS2Manager.getIdentifier(msg.channel, msg)
      const pSubIdentifier = WS2Manager.getIdentifier(pSub[0], pSub[1])
      if (msg.channel === pSub[0] && msgIdentifier === pSubIdentifier) {
        this.managedUnsubscribe(msg.channel, msgIdentifier)
        resolve()
      }
    }

    const unsubCb = (pChanId, resolve) => (msg = {}) => {
      const { chanId } = msg
      if (`${chanId}` !== pChanId) { return }
      resolve()
    }

    const pendingSubscribeP = _flatten(this._sockets.map(s =>
      s.pendingSubscriptions.map(pSub => {
        let cb
        return new Promise((resolve) => {
          cb = subCb(pSub, resolve)
          s.ws.on('subscribed', cb)
        }).then(() => s.ws.removeListener('subscribed', cb))
      })
    ))

    await Promise.all(pendingSubscribeP)

    const pendingUnsubscribeP = _flatten(this._sockets.map(s =>
      s.pendingUnsubscriptions.map(pChanId => {
        let cb
        return new Promise((resolve) => {
          cb = unsubCb(pChanId, resolve)
          s.ws.on('unsubscribed', cb)
        }).then(() => s.ws.removeListener('unsubscribed', cb))
      })
    ))

    await Promise.all(pendingUnsubscribeP)

    return Promise.all(this._sockets.map(socket => socket.ws.close()))
  }

  /**
   * Returns identifier symbol from provided object
   *
   * @returns {string} s
   */

  static getIdentifier (channel, body) {
    if (channel === 'candles') { return body.key }
    return body.symbol
  }
   ...
   */
  onCandle ({ key, cbGID, emitHeartBeat }, cb) {
    ...
    s.ws.onCandle({ key, cbGID, emitHeartBeat }, cb)
  }
